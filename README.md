# .mui-patcher
Simple script that uses Resource Hacker to find all .mui files in a folder and patch them

## Prerequisites
Shell file: Wine, [ResourceHacker.exe](https://www.angusj.com/resourcehacker/resource_hacker.zip) placed next to the script, python (python3) if you want some log files to not be garbled.
Executable: Wine or Windows, same ResourceHacker.exe position.

## Operation
Either execute the script in the folder wanted, or pass it as a variable. It creates a `logs` directory in whitch logs for every patching sequence is stored.

By default it will search 5 subfolders deep for the .mui file (and thus 4 layers for the .exe/.dll file), then match them case insensitive-ly (chess.exe -> Chess.exe.mui)
